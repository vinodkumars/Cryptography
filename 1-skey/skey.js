...
// We used the algorithm described in Jackobsson's algorithm and made space optimizations as discussed in it. We used a destination-prioritised queue for the pebbles.
*/
...
// S/KEY can be used as a second factor one time password authentication. It is desirable when the following advantages are preferred:
// 1)Protection against passive eavesdropping
// 2)It is easy to use and implement and has no secret algorithm
// 3)It has no stored secrets
// The main disadvantage is that a new secret key has to be setup when the chain is fully used.
*/

/* 3. (Will not affect your grade:) How long did you spend on this project?
// Problem statement + Paper reading, high level design of the solution: 8 Hrs
// Javascript learning curve: 4 Hrs
// Coding: 4 Hrs
// Testing: 2 Hrs
// Total: 18 Hrs
*/
...
// It would be great if one had the liberty to use any language he/she is comfortable with, as it eliminates the time taken to learn the new language.
*/
...
function pebble_chain() {
  ...
  //The queue has smaller destination elements on the front. In other words, smaller destination has higher priority.
  ...
  //Peeking the first pebble (but not dequeueing)
  chain.peekQueue=function(){
      return chain.state.pebbleQueue[0];
      }
  ...
  chain.advance = function() {
    ...
    if(chain.state.position%2===1)
    {
        ...
        return hash(chain.peekQueue().value);
    }
    else
    {
